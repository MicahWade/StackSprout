...
function renderStack() {
  return generatedStack.map((tech, i) => {
    ...
    return `
      <div class="coolors-row min-h-[100px] flex items-center px-8 bg-slate-900/80 border border-slate-800/50 rounded-3xl relative group overflow-hidden transition-all hover:border-slate-700 shadow-xl shrink-0 scramble-effect" style="animation-delay: ${i * 100}ms">
        ...
        <div class="flex items-center gap-6 z-10 w-full">
          <div class="w-14 h-14 p-3 bg-slate-950 rounded-2xl border border-slate-800 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform overflow-hidden svg-container">${tech.svgContent}</div>
          <div class="flex-1"><h3 class="text-xl font-black uppercase tracking-tighter text-white group-hover:text-brand-orange transition-colors line-clamp-1">${tech.name}</h3><p class="text-[9px] font-black opacity-40 uppercase tracking-[0.2em] text-slate-400 mt-1">${tech.category}</p></div>
